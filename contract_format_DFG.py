...
class SourceFormatterAndDFG:
    def __init__(self, input_file=None):
        ...
        self.dict_data = {}
        self.code_tokens = []
        self.var_list = ['msg.sender']
        self.function_limit = ["private"]
        self.var_op_bool = ['!', '~', '**', '*', '!=', '<', '>', '<=', '>=', '==', '<<', '>>', '||', '&&']
        self.var_op_assign = ['|=', '=', '^=', '&=', '<<=', '>>=', '+=', '-=', '*=', '/=', '%=', '++', '--']
        ...
        if isinstance(json_data, dict):
            if json_data.get("type") == "EventDefinition":
                ...
                if (json_data.get("type") == "FunctionDefinition" or json_data.get("type") == "ModifierDefinition"
                    ...
                    if json_data.get("type") == "ModifierDefinition":
                        self.function_limit.append(f"FUN{self.n_fun}")
                    self.n_fun += 1
                if (json_data.get("type") == "VariableDeclaration" or json_data.get("type") == "Parameter"
                    ...
                    if json_data.get("typeName") and json_data.get("typeName").get("type") == "Mapping":
                        self.var_list.append(f"VAR{self.n_var}")
                    self.n_var += 1
    ...
    def remove_multiple_spaces(self):
        # Use re.MULTILINE flag to make the pattern match across multiple lines
        self.source_code = re.sub(r' +', ' ', self.source_code, flags=re.MULTILINE)
        self.source_code = re.sub(r'[ \t]+', ' ', self.source_code, flags=re.MULTILINE)
